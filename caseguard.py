...
#
# To enable the "caseguard" extension globally, put these lines in your ~/.hgrc:
#  [extensions]
#  caseguard = /path/to/caseguard.py
#
# You may optionally add a section in the config file that specifies what options
# you want to have always enabled:
#
# [caseguard]
# silent = true
# override = true
#
# Please note that having override always enabled will revert the add command to
# its normal behaviour, and caseguard will not block you from adding the files to
# the repository. However, if silent = false, you will still get a listing of the
# files that will cause problems.
#
...
def uisetup(ui):    
    def reallyadd(orig, ui, repo, override=False, silent=False, *pats, **opts):
    ...
    entry[1].append(('', 'override', False, _('add files regardless of possible case-collision problems')))
    entry[1].append(('','silent', False,_('do not output a list of case-colliding files')))
